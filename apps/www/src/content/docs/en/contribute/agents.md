...
Review submission is human-authorized by default. One repository-local standing exception, `proto-ui-scheduled-review-v1`, lets only the maintainer-controlled Codex schedule whose trusted v2 handoff carries `executionTaskId: proto-ui` submit complete, finding-backed `REQUEST_CHANGES`; a missing or different task identity fails closed. It may submit `APPROVE` only for a clean packet with successful live checks when no current or previous changed-file path names a YAML entity under the nine `spec/**` entity collections. An otherwise approvable spec-entity change stops for maintainer review. The exception excludes self-review, `COMMENT`, `ABSTAIN`, merge, ready-for-review, close, labels, assignment, publication, release, access, and rulesets.
