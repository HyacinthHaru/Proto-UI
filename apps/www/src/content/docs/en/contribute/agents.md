...
Review submission currently requires human authorization. The intended local schedule scope `proto-ui-scheduled-review-v1` is `pending-runtime-identity`, so the task remains read-only and sends every `REQUEST_CHANGES` or `APPROVE` recommendation to the maintainer. Its intended future scope would allow complete, finding-backed `REQUEST_CHANGES`, and `APPROVE` only for a clean packet with successful live checks when no current or previous changed-file path names a YAML entity under the nine `spec/**` entity collections. That scope cannot become active until the submission boundary can verify repository-and-task-bound runtime identity; a public task ID is not proof.

Local review is always available. A low-band Agent working with you may return a partial review or `ABSTAIN` with clear limitations. Submitting that review to GitHub is a separate human-assisted action and requires your authorization plus a credential with permission; an autonomous run stops first. The `submit-review` path re-collects the whole canonical input (body, commits, top-level conversation comments, replies, threads, checks) live from GitHub and rejects a packet whose digest no longer matches; it also derives the viewer identity, pull-request author, and credential permission from that live context instead of trusting caller-provided strings. When authorized, the same command performs the GitHub Review API write with `commit_id` bound to the reviewed packet head and validates that binding in the receipt. A separate later `gh pr review` write is not supported.
