...
Compare the initially empty group and the disabled group, then repeat in Web Components, React, and Vue. The normal control is 16px with an 8px decorative glyph box. Keyboard focus adds a three-pixel ring; only color and box-shadow transition, over 150ms. Changing the website theme changes the input tint without changing selection.
