...
6. Validate the v2 input against `internal/agent-operations/schemas/review-input.schema.json` and compute its digest with `pnpm agent:review -- input-digest --input <review-input-path>`. The canonical input includes PR state and draft state, current and previous changed-file paths, commits, existing reviews, top-level conversation comments, replies, threads, checks, and external evidence. Bind the review packet to the same repository, PR, base SHA, head SHA, returned digest, timestamp, authorized scope, affected entities and surfaces. Treat later commits as an incremental review range. Skip a same-head packet only when its canonical input digest and review class are also unchanged.
...
Never merge. Do not otherwise change GitHub state or fix the change unless the action has separate current-user or standing authorization. Immediately before a review write, run `pnpm agent:review -- submit-review --packet <path> --input <review-input-path> --handoff <validated-handoff-path> [--assessment <result-path>] [--external-evidence-file <evidence.json>] --authorization <explicit-current-user|proto-ui-scheduled-review-v1>`. This is the only supported mutation path: it re-collects the whole canonical review input live from GitHub with `gh`, compares its digest against `reviewInputDigest`, derives PR state, draft state, current and previous changed-file paths, existing reviews, top-level conversation comments, viewer identity, pull-request author, credential permission, and CI conclusion, then submits through the GitHub Review API with `commit_id` fixed to the packet head. It validates the returned review commit before issuing a receipt. Do not follow a preflight with `gh pr review` or any other unbound review write. Caller-provided identities, permissions, file classifications, and CI conclusions are never accepted. Any drift or pagination overflow fails closed. `externalEvidence` cannot be re-collected live: pass the exact recorded array with `--external-evidence-file`, otherwise a packet recorded with external evidence fails the digest check.

`proto-ui-scheduled-review-v1` records the maintainer's intended standing scope, but its status is `pending-runtime-identity`. Until the submission boundary can verify a repository-and-task-bound runtime identity, an `autonomous` schedule must not submit `REQUEST_CHANGES`, `APPROVE`, or any other GitHub review. It may complete the read-only review and send the maintainer a decision package. The intended future scope permits complete, finding-backed `REQUEST_CHANGES`, and permits `APPROVE` only for a clean packet with successful live checks and no current or previous changed-file path naming a spec entity under `spec/{contracts,prototypes,modules,adapters,decisions,host-caps,tests,versions,knowledge}/*.yaml`; this intent is not currently executable authority. The pending authorization excludes `COMMENT`, `ABSTAIN`, ready-for-review, merge, close, labels, assignment, publication, release, access, and rulesets.
