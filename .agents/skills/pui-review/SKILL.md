...
6. Validate the v2 input against `internal/agent-operations/schemas/review-input.schema.json` and compute its digest with `pnpm agent:review -- input-digest --input <review-input-path>`. The canonical input includes PR state, draft state, base ref name, current and previous changed-file paths, commits, existing reviews, top-level conversation comments, replies, threads, checks, and external evidence. Bind the review packet to the same repository, PR, base ref name, base SHA, head SHA, returned digest, timestamp, authorized scope, affected entities and surfaces. Treat later commits as an incremental review range. Skip a same-head packet only when its canonical input digest and review class are also unchanged.
