...
const tabsList = definePrototype<BrutalistTabsListProps, BrutalistTabsListExposes>({
  ...
  setup(def) {
    ...
    def.feedback.style.use(
      tw(
        'inline-flex h-11 items-center rounded-none border-2 border-foreground bg-secondary-background p-1 text-foreground shadow-[3px_3px_0_0_var(--pui-foreground)]'
      )
    );
  },
});
