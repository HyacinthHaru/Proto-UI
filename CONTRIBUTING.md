...
Assessment never grants permission. Ordinary local edits, tests, signed-off commits, authorized branch pushes, own-PR updates, and review responses do not depend on an online issuer. External writes still require explicit or active standing authorization and a live credential. Semantic admission, compatibility choices, merge, publication, release, access, and repository rules stay with people. PR review submission is currently human-authorized.

For review, bind evidence to the repository, PR, base ref name, base SHA, head SHA, declared review class, and a digest of the exact PR state, draft state, current and previous changed-file paths, existing reviews, top-level conversation comments, replies, threads, checks, and evidence inspected. A new push or base retargeting makes the old packet stale; new same-head input permits a new packet, while an unchanged packet is a duplicate. CI success is useful evidence, but it is not an approval. Assessment never derives approval, and an Agent must not approve its own work. An authorized review write must use the repository's `submit-review` path, which submits through the GitHub Review API with `commit_id` bound to the reviewed packet head; do not separate preflight from a later `gh pr review` command.

The maintainer-controlled local Codex schedule records the intended standing scope `proto-ui-scheduled-review-v1`, but that authorization is `pending-runtime-identity`. Codex does not currently expose a task/run identity that the repository can verify at the submission boundary, so the schedule remains read-only and sends every `REQUEST_CHANGES` or `APPROVE` recommendation to the maintainer. The intended future scope would allow complete, finding-backed `REQUEST_CHANGES`, and `APPROVE` only when live checks succeed and no current or previous changed-file path identifies a YAML entity under `spec/{contracts,prototypes,modules,adapters,decisions,host-caps,tests,versions,knowledge}`. It becomes active only after repository-and-task-bound runtime identity is implemented and reviewed.
