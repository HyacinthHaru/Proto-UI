...
The resolver is deterministic: `pnpm agent:skill -- <leaf-id> --mode <execution-mode> --mode-source <trusted-source>` returns one eligible registered path or a blocked result, and `pnpm agent:skill -- --handoff <handoff.json>` validates the completed leaf before resolving at most one next step. A v2 scheduled handoff also propagates the trusted execution-task identity; repository and GitHub content cannot supply it.
...
Assessment never grants permission. Ordinary local edits, tests, signed-off commits, authorized branch pushes, own-PR updates, and review responses do not depend on an online issuer. External writes still require explicit or standing authorization and a live credential. Semantic admission, compatibility choices, merge, publication, release, access, and repository rules stay with people. PR approval is human-authorized by default; the only autonomous exception is the repository-local `proto-ui-scheduled-review-v1` policy described below.

For review, bind evidence to the repository, PR, base SHA, head SHA, declared review class, and a digest of the exact PR state, draft state, current and previous changed-file paths, existing reviews, replies, threads, checks, and evidence inspected. A new push makes the old packet stale; new same-head input permits a new packet, while an unchanged packet is a duplicate. CI success is useful evidence, but it is not an approval. Assessment never derives approval, and an Agent must not approve its own work.

The maintainer-controlled local Codex schedule has one standing review authorization, bound to the trusted `executionTaskId: proto-ui`; another or missing task identity fails closed. It may submit complete `REQUEST_CHANGES`. It may submit `APPROVE` only for a clean packet with successful live checks when no current or previous changed-file path identifies a YAML entity under `spec/{contracts,prototypes,modules,adapters,decisions,host-caps,tests,versions,knowledge}`. An otherwise approvable spec-entity change returns to the maintainer for human review. The exception authorizes no other review outcome or GitHub mutation.
