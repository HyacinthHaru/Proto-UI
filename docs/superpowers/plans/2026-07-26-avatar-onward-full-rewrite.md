...
> **Maintainer scope freeze (2026-07-26):** PR #323 is an incubation branch, not one merge unit. Base Input, Base Textarea, and provisional portable Native Control work are deferred pending dedicated protocol research. Do not execute Tasks 1–2, any Native Control prerequisite, or editor styling tasks from this plan in this branch; demos that need editing must use host-native `<input>`/`<textarea>` as application details, not Base prototypes. Remaining work must stop at stable CI/package/browser evidence and be split into focused PRs.
