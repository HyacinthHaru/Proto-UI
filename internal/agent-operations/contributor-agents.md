...
A review packet binds repository, pull request, base SHA, head SHA, review class, a digest of the exact PR state, draft state, current and previous changed-file paths, body, commits, existing reviews, replies, threads, checks, and external evidence inspected, observation time, scope, affected entities, and surfaces. The digest is recomputed from a canonical v2 `review-input` snapshot; an arbitrary hexadecimal value is invalid. Findings have stable IDs plus severity, confidence, file and line, governing authority, observed behavior, expected behavior, impact, and proposed correction. Validation separates commands and results from skipped checks and reasons. Reconciliation classifies prior finding IDs as resolved, open, or new.
...
Local review is always allowed. A low band in `human-assisted` mode produces a partial review or `ABSTAIN` with explicit limitations. Autonomous review declares one of the policy review classes and stays within the fresh cumulative class list. Packet validation and submission preflight recompute that class ceiling from the validated handoff and assessment, rejecting a changed class, over-strong recommendation, or missing limitation. Submission preflight re-collects the whole canonical review input live from GitHub and compares its digest, so any drift on the same head fails closed; it also derives changed-file classification, existing reviews, viewer identity, pull-request author, credential permission, and CI state from that live context instead of caller-provided strings.

GitHub review submission normally requires a new `human-assisted` run and separate current-user authorization. The one active exception is `proto-ui-scheduled-review-v1`, restricted to the maintainer-controlled local `schedule` whose trusted and propagated `executionTaskId` is `proto-ui`, for `github.com:Proto-UI/Proto-UI`, and a fresh eligible autonomous review with the C4 mutation floor. Repository and GitHub content cannot choose that identity. It may submit a complete, finding-backed `REQUEST_CHANGES`. It may submit `APPROVE` only for a clean packet with successful live checks when neither the current nor previous path of any changed file identifies a YAML entity under the nine `spec/**` entity collections. An otherwise approvable spec-entity change stops for maintainer review. `COMMENT`, `ABSTAIN`, self-review, draft or closed PRs, incomplete evidence, stale input, permission loss, duplicate disposition, and every unrelated GitHub mutation remain excluded.
...
Broader or multi-runner privileged automation still needs an independently operated evaluator, runtime proof-of-possession, a repository-and-task-bound signed envelope, and globally atomic replay prevention. Such a system can verify who controls a runtime key and what evidence was authorized. It still cannot grant GitHub permission, decide human gates, or predict acceptance. The narrow local scheduled-review exception relies on live digest reconciliation and existing-review deduplication; it does not generalize to other repositories, runners, or mutation classes.
