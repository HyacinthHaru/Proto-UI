...
GitHub review submission currently requires a new `human-assisted` run and separate current-user authorization. The intended local schedule scope `proto-ui-scheduled-review-v1` is `pending-runtime-identity`: a schedule may perform read-only review and prepare a maintainer decision package, but it may not submit `REQUEST_CHANGES`, `APPROVE`, or another review. Its intended future scope would allow complete, finding-backed `REQUEST_CHANGES`, and `APPROVE` only for a clean packet with successful live checks when neither the current nor previous path of any changed file identifies a YAML entity under the nine `spec/**` entity collections. Activating that scope requires a repository-and-task-bound runtime identity that is verified at the submission boundary; a public task ID or authorization ID is not proof.
...
Privileged automation needs an independently operated evaluator, runtime proof-of-possession, a repository-and-task-bound signed envelope, and globally atomic replay prevention. Such a system can verify who controls a runtime key and what evidence was authorized. It still cannot grant GitHub permission, decide human gates, or predict acceptance. Live digest reconciliation and existing-review deduplication remain necessary but cannot identify the invoking desktop task by themselves.
