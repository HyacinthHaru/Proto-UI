...
- The integrated release rehearsal passes release identity/assets, catalog, types, release tests, runtime tests, spec snapshot generation, the eight-package launch-governance scan, 40/40 registry identity readiness, package build and publish dry-run, React and CLI tarball consumers, the production documentation build, manifest checks, and Agent-document validation. `@proto.ui/module-text-control` now resolves at a deprecated, code-free `0.0.0-bootstrap.0` identity and has a package-scoped Trusted Publisher for the protected release workflow.
...
- This draft does not mean rc.7 is installable. Before activation, review and merge the preparation PR, rerun the protected publication workflow from the resulting `main` commit, publish the complete 40-package set, and then establish and verify the immutable spec snapshot, Git tag, GitHub prerelease, npm `next` state, and publication evidence.
